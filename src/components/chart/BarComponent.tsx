...
export const BarComponent = ({ dataKey, xOffset, aggregatedData, isFact, fill }: BarComponentType) => {
    return (
        <Bar dataKey={dataKey} fill={fill}>
            {isFact && aggregatedData.map((entry, index) => {
            ...
            <LabelList dataKey={dataKey} position="top" fill="var(--black)" content={({ value, x, y }) => {
                if (value === 0) {
                    return (
                        <>
                            {
