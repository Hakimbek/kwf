...
export const Client = () => {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToClient(setRowData);
    return () => unsubscribe();
  }, []);
  ...
  const onCellValueChanged = async (event: CellValueChangedEvent) => {
    ...
    try {
      await updateClient(data.id, { name: trimmedValue });
    } catch {
  ...
  const handleDelete = async (data: IClient) => {
    try {
      await deleteClient(data.id);
      toggle();
    } catch {
      toast.error("Client is used in other collections!");
    }
  };
  ...
  const columnDefs = useMemo<ColDef<IClient>[]>(
    () => [
      ...
      },
      {
        ...
        width: 80,
        resizable: false,
        headerClass: "client-header-cell",
